...
import {
  ...
  TrendingUp, Zap, Target, BookOpen
} from 'lucide-react';
...
// ─── Sparkline data ───
const sparkData = {
  leads:    [5, 7, 6, 9, 8, 11, 12].map((v, i) => ({ v })),
...
const statsCards = [
  ...
  },
  {
    ...
    sparkColor: '#3B82F6',
    icon: FileText,
    link: '/projects?status=הצעה נשלחה',
  },
  {
    label: 'פרויקטים משוריינים',
    value: 28,
    change: '+10%',
    changePositive: true,
    iconBg: '#FAF5FF',
...
const pipelineStages = [
  { label: 'לידים', value: 12, color: '#3B82F6', bg: '#EFF6FF' },
  { label: 'בניית הצעה', value: 45, color: '#F59E0B', bg: '#FFFBEB' },
  { label: 'נשלחו ללקוח', value: 32, color: '#8B5CF6', bg: '#F5F3FF' },
  { label: 'אושרו', value: 28, color: '#22C55E', bg: '#F0FDF4' },
  { label: 'בביצוע', value: 18, color: '#FF8C00', bg: '#FFF7ED' },
];
const pipelineMax = Math.max(...pipelineStages.map(s => s.value));

// ─── Revenue Ring ───
const revenueTarget = 500000;
const revenueCurrent = 362000;
const revenueProfit = 97400;
const revenuePercent = Math.round((revenueCurrent / revenueTarget) * 100);
const profitMargin = Math.round((revenueProfit / revenueCurrent) * 100);

// ─── Urgent Tasks ───
const urgentTasks = [
  {
    id: '4833-24',
    title: 'טיול חברה - הייטק בע"מ',
    badge: 'דחוף',
    badgeColor: '#dc2626',
    badgeBg: '#fef2f2',
    detail: 'מחיר בהערכה',
  ...
  {
    id: '4831-24',
    title: 'נופש שנתי - אילת',
    badge: null,
    badgeColor: '',
    badgeBg: '',
    detail: 'מסמך ביטוח פג תוקף',
    detailColor: '#dc2626',
...
// ─── Animated Stat Card ───
function StatCard({ stat, index, sparkKey }: { stat: typeof statsCards[0]; index: number; sparkKey: typeof sparkKeys[number] }) {
...
function PipelineStage({ stage, index, maxVal }: { stage: typeof pipelineStages[0]; index: number; maxVal: number }) {
  const counter = useCountUp(stage.value, 1400);
  const widthPercent = Math.max((stage.value / maxVal) * 100, 18);
  ...
  return (
    ...
    >
      ...
      {index < pipelineStages.length - 1 && (
        <ChevronLeft size={14} className="text-[#ddd6cb] shrink-0" />
      )}
      {index === pipelineStages.length - 1 && (
        ...
        >
          ...
          <div className="w-full mt-4 space-y-2">
            <div className="flex justify-between text-[13px]">
              <span className="text-[#8d785e]">הכנסות החודש</span>
              <span
            ...
            <div className="flex justify-between text-[13px]">
              <span className="text-[#8d785e]">רווח</span>
              <span
            ...
            <div className="flex justify-between text-[13px]">
              <span className="text-[#8d785e]">שולי רווח</span>
              ...
              >
                <div className="flex items-center justify-between px-6 py-5">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <button className="p-2 rounded-lg hover:bg-[#f5f3f0] transition-colors text-[#8d785e]">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[#8d785e] mt-1">{item.time}</p>
                    </div>
                  </motion.div>
                );
              })}
